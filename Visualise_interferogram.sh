...
# This script takes a Drummer-produced wrapping interferogram (.flat), and a Drummer-produce SLC, both with dem .rsc files, and produces a map of the interferogram draped on the SLC.
#
# Usage: Visualise_flatfile flatfilenam flat_demfilename slcfilename slc_demfilename 		where slcfilename is 'NAME.geo.amplitude.nc' (having been put into amplitude over on Drummer and nc previously).
...
if [[ $input == "Y" || $input == "y" ]]; then
		...
		gmt xyz2grd $slcfilename -G$slcfilename.nc -I$nolons+/$nolats+ $reg -ZTLd -Vl
...
echo -e "Cutting input data to exclude the greatest 3% of values by frequency (should do this if image is otherwise just black, but can take bloody ages)."
echo "	Do that? [Y,n]"
read input
if [[ $input == "Y" || $input == "y" ]]; then
	gmt grdhisteq $slcfilename.nc -C30 -D -Vl > levels.tmp
	max=$(tail -n1 levels.tmp | cut -f1)
else
	max=$(gmt grdinfo -T $slcfilename.nc | cut -f2 -d'/')
    echo "	Moving on; we will plot all the data in a linear scale..on your head be it."
fi
echo -e " "

gmt grd2cpt $slcfilename.nc -Cgray -L0/$max -S20 -V > shades.cpt.tmp
